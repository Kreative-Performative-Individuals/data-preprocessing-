...
anomaly_indices = np.random.choice(
    range(n_points_historical),
    size=int(n_points_historical * anomaly_probability),
    replace=False,
)
magg_factor = np.random.uniform(
    1.2, 1.5, size=int(n_points_historical * anomaly_probability)
)
window_size = 15

normal_data.append(
    np.convolve(
        np.random.normal(60000, 9000, n_points_historical),
...
normal_data.append(
    np.convolve(
        np.random.normal(31100, 9000, n_points_historical),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # avg
...
normal_data.append(
    np.convolve(
        np.random.normal(15000, 8000, n_points_historical),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # min
...
normal_data.append(
    np.convolve(
        np.random.normal(44200, 3000, n_points_historical),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # max
...
normal_data.append(
    np.convolve(
        np.random.normal(35000, 8000, n_points_historical),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # var
...
historical_data = {
    ...
    * len(times[window_size:-window_size]),  # Added asset_id column
    ...
    * len(times[window_size:-window_size]),  # Added operation column
...
non_nan_indices = []
magg_factor = np.random.uniform(
    1.2, 1.5, size=int(n_points_stream * anomaly_probability)
)

normal_data.append(
    np.convolve(
        np.random.normal(60000, 9000, n_points_stream),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # sum #86400
normal_data[0] = np.array(
    [
        value + increment if index > drift_index else value
        for index, value in enumerate(normal_data[0])
    ]
)
...
normal_data.append(
    np.convolve(
        np.random.normal(31100, 9000, n_points_stream),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # avg
normal_data[1] = np.array(
    [
        value + increment if index > drift_index else value
        for index, value in enumerate(normal_data[1])
    ]
)
nan_mask = np.random.rand(n_points_stream) < probability_of_nan
nan_mask[0] = False
...
normal_data.append(
    np.convolve(
        np.random.normal(15000, 8000, n_points_stream),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # min
normal_data[2] = np.array(
    [
        value + increment if index > drift_index else value
        for index, value in enumerate(normal_data[2])
    ]
)
nan_mask = np.random.rand(n_points_stream) < probability_of_nan
nan_mask[0] = False
...
normal_data.append(
    np.convolve(
        np.random.normal(44200, 7000, n_points_stream),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # max
normal_data[3] = np.array(
    [
        value + increment if index > drift_index else value
        for index, value in enumerate(normal_data[3])
    ]
)
nan_mask = np.random.rand(n_points_stream) < probability_of_nan
nan_mask[0] = False
...
normal_data.append(
    np.convolve(
        np.random.normal(35000, 8000, n_points_stream),
        np.ones(window_size) / window_size,
        mode="same",
    )
)  # var
normal_data[4] = np.array(
    [
        value + increment if index > drift_index else value
        for index, value in enumerate(normal_data[4])
    ]
)
nan_mask = np.random.rand(n_points_stream) < probability_of_nan
nan_mask[0] = False
...
possible_anomaly_indeces = list(
    set(non_nan_indices[0]).intersection(
        non_nan_indices[1], non_nan_indices[2], non_nan_indices[3], non_nan_indices[4]
    )
)
anomaly_indices = np.random.choice(
    possible_anomaly_indeces,
    size=int(n_points_stream * anomaly_probability),
    replace=False,
)
normal_data[0][anomaly_indices] = max(normal_data[0]) * magg_factor
normal_data[1][anomaly_indices] = max(normal_data[1]) * magg_factor
